...
async function fetchAndStoreVideos(request, reply) {
  ...
  try {
    ...
    if (!response.data || !response.data.results || response.data.results.length === 0) {
      reply.status(404).send({ message: 'Nenhum vídeo encontrado para esta série.' });
      return;
    }
    ...
    if (videos.length === 0) {
      reply.status(400).send({ message: 'Nenhum vídeo válido encontrado para inserir.' });
      return;
    }
    ...
    reply.status(201).send({ message: 'Vídeos inseridos com sucesso!', videos });
  } catch (error) {
  ...
  async function getVideos(request, reply) {
    ...
    try {
      ...
      let query = knex('videos')
        .select('id', 'show_id', 'name', 'key', 'site', 'size', 'type', 'official', 'published_at')
        .limit(limit)
        .offset(offset);
