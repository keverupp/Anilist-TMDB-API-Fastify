...
exports.up = async function (knex) {
  ...
  // 4) Remove the old check constraint
  await knex.raw(`
    ALTER TABLE reactions
    DROP CONSTRAINT IF EXISTS reactions_type_check;
  `);

  // 5) descarta o enum antigo (check if it exists first)
  ...
  // 6) renomeia o enum novo para o nome original
  await knex.raw(`
...
exports.down = async function (knex) {
  ...
  // Optionally, you might want to re-create the old check constraint in the down migration
  // await knex.raw(`
  //   ALTER TABLE reactions
  //   ADD CONSTRAINT reactions_type_check CHECK (type IN ('like', 'dislike'));
  // `);
};
